...
## Closed source-authority debt

### Gnarl's Ramblings: CLOSED

A generalized Gnarl lifecycle-commentary subsystem is implemented with separate persisted presentation state. It supports authored objective clarification, finite reminder pools, objective updates, warnings, branch framing, success/failure reactions and delayed post-quest commentary. Coverage deliberately includes major campaign transitions, all six core magic families, all nine dedicated adventure families and the End campaign. A dedicated validator guards the contract.
...
Existing local provider logic remains authoritative for the designated quest context. No fixed final-world coordinates are required by the completion contract.
