...
Status: DERIVED FROM APPROVED DECISIONS. Companion to `V5_CAMPAIGN_SYSTEM_AUTHORITY.md` §7.5.

Total provider quests: **43** across 11 civilizations.

These quests are handled entirely by the Villager Retaliation provider layer. **They have no Questlog entries and no presenter popups.** Questlog carries only two entries per civilization: the anchor quest and the civilization-state quest.

A provider quest never sets a terminal state directly. It writes a **named fact**. The civilization-state quest reads those facts.
