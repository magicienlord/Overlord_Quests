...
Visual states are the five universal states (authority §5.2). Registers are per presenter (§5.3). `Order (presenter voice)` is `TO BE AUTHORED` on every quest by design (Q242). `Reward` is `NONE` on every quest (Q241) — which does **not** mean an empty `rewards` array, see `V5_IMPLEMENTATION_SPECS.md` §1.
