...
This protocol validates the temporary NPC sidequest provider scaffold. The fixture and commands below are not OVERLORD REIGN story canon.
...
- no story quest, civilization state, or production reward is introduced by the fixture.
...
Then sneak and interact with Provider A using the main hand. The provider screen must open and show `[DEV] NPC Provider Prototype` as available for acceptance.

Select the accept action. The same screen should refresh from the server and show the quest as in progress. The controls must not remain permanently disabled after the response.
...
Reset the fixture and accept it again if necessary. Open Provider A's menu, then move more than 8 blocks away while the screen remains open. The menu must close automatically.
...
The provider scaffold passes this milestone only when the server-authoritative accept, persistence, same-provider turn-in, refresh, ordinary-interaction preservation, and distance lifecycle behaviors all work in the actual Forge 1.20.1 instance.
